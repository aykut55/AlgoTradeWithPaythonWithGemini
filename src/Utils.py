class CUtils:
    ...
    def yukari_kesti(self, i, ListX, ListY, esitlikDahil=True):
        if esitlikDahil:
    ...
    def asagi_kesti(self, i, ListX, ListY, esitlikDahil=True):
        if esitlikDahil:
            res = ListX[i - 1] > ListY[i - 1] and ListX[i] <= ListY[i]
        else:
            res = ListX[i - 1] > ListY[i - 1] and ListX[i] < ListY[i]
    ...
    def yukari_kesti_seviye(self, i, ListX, Seviye, esitlikDahil=True):
        if esitlikDahil:
    ...
    def asagi_kesti_seviye(self, i, ListX, Seviye, esitlikDahil=True):
        if esitlikDahil:
            res = ListX[i - 1] > Seviye and ListX[i] <= Seviye
        else:
            res = ListX[i - 1] > Seviye and ListX[i] < Seviye
    ...
    def buyuk(self, i, ListX, ListY):
        return ListX[i] > ListY[i]

    def kucuk(self, i, ListX, ListY):
        return ListX[i] < ListY[i]

    def buyuk_esit(self, i, ListX, ListY):
        return ListX[i] >= ListY[i]

    def kucuk_esit(self, i, ListX, ListY):
        return ListX[i] <= ListY[i]

    def esit(self, i, ListX, ListY):
        return ListX[i] == ListY[i]

    def esit_degil(self, i, ListX, ListY):
        return ListX[i] != ListY[i]

    def ustunde(self, i, ListX, ListY):
        return self.buyuk(Sistem, i, ListX, ListY)

    def altinda(self, i, ListX, ListY):
        return self.kucuk(Sistem, i, ListX, ListY)

    def buyuk_coklu(self, i, *args):
        res = True
    ...
    def liste_topla(self, Source, K):
        return [x + K for x in Source]

    def liste_cikar(self, Source, K):
        return [x - K for x in Source]

    def liste_carp(self, Source, K):
        return [x * K for x in Source]

    def liste_bol(self, Source, K):
        return [x / K for x in Source]

    def liste_yuzde(self, Source, K, Offset=0):
        return [x * K / 100 + Offset for x in Source]

    def liste_yuzde_liste(self, Source, K, OffsetList=None):
        if OffsetList:
            return [Source[i] * K / 100 + OffsetList[i] for i in range(len(Source))]
        else:
            return [x * K / 100 for x in Source]

    def liste_topla_liste(self, Source1, Source2):
        return [Source1[i] + Source2[i] for i in range(len(Source1))]

    def liste_cikar_liste(self, Source1, Source2):
        return [Source1[i] - Source2[i] for i in range(len(Source1))]

    def liste_elemanlarini_resetle(self, begIndex, barCount, lst, lowerLimit, upperLimit):
        return self.liste_elemanlarini_set_et(Sistem, begIndex, barCount, lst, lowerLimit, upperLimit, 0)

    def liste_elemanlarini_set_et(self, begIndex, barCount, lst, lowerLimit, upperLimit, newValue=0):
        newList = list(lst)
    ...
    def get_new_list(self, Count, Value):
        return [Value] * Count

    def get_zero_level_list(self, Count):
        return self.get_new_list(Sistem, Count, 0.0)

    def get_new_list_from_src(self, SrcList):
        return list(SrcList)

    def get_distance_list_as_percentage(self, Source, High, Low):
        return [(Source[i] - Low[i]) * 100 / (High[i] - Low[i]) for i in range(len(Source))]

    def get_distance_list_as_percentage_ref(self, Src, Ref):
        return [(Src[i] - Ref[i]) / Ref[i] * 100 for i in range(len(Src))]

    def get_kairi(self, Src, Ref):
        return self.get_distance_list_as_percentage_ref(Sistem, Src, Ref)

    def get_kairi_ma(self, Src, MaPeriyot, MaYontem="Simple"):
        return self.get_distance_list_as_percentage_ref(Sistem, Src, Sistem.MA(Src, MaYontem, MaPeriyot))

    def sort_the_signals2(self):
        return Sistem.Liste(Sistem.BarSayisi, 0)

    def get_the_list_sort_status(self, i, *args):
        statusDict = {}
    ...
    def create_levels(self, MaxLevelNumber=100, UseFloatingKeys=False):
