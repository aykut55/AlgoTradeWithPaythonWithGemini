...
class CTimeUtils(CBase):
    ...
    def gecen_zaman_bilgilerini_al(self):
    ...
    def is_yeni_ay(self, BarIndex):
        i = BarIndex
        return self.V[i].Date.month != self.V[i - 1].Date.month

    def is_yeni_hafta(self, BarIndex):
        i = BarIndex
        yeni_gun = self.V[i - 1].Date.day != self.V[i].Date.day
        return yeni_gun and self.get_week_number(self.V[i - 1].Date) != self.get_week_number(self.V[i].Date)

    def is_yeni_gun(self, BarIndex):
        i = BarIndex
        return self.V[i].Date.day != self.V[i - 1].Date.day

    def is_yeni_saat(self, BarIndex):
        i = BarIndex
        return self.V[i].Date.hour != self.V[i - 1].Date.hour

    def is_yeni(self, BarIndex, Periyot):
        result = False
        i = BarIndex
        if Periyot == "5S":
            pass
            ...
            result = self.is_yeni_gun(i)
        elif Periyot == "H":
            result = self.is_yeni_hafta(i)
        elif Periyot == "M":
            result = self.is_yeni_ay(i)
        else:
            result = False
        return result

    def gecen_sure(self, Ref="A"):
        sure = 0.0
        if Ref == "A":
    ...
    def periyot_carpan_hesapla(self, Periyot1, Value1, Periyot2):
        k1 = self.periyot_carpan_hesapla(Periyot1)
        k2 = self.periyot_carpan_hesapla(Periyot2)
        value2 = Value1 * k2 / k1
        return value2

    def periyot_carpan_hesapla(self, Periyot):
        ...
        if Periyot == "B":
            ...
            print("Sistem.Periyot tanımlı değerler arasından seçilmelidir! ")
        return periyotCarpanGunluk

    def start_watch_dog_counter(self, Counter):
        self.WatchDogCounter = Counter
        self.WatchDogCounterStarted = True
        self.WatchDogCounterFinished = False

    def stop_watch_dog_counter(self):
        self.WatchDogCounterStarted = False

    def watch_dog_counter_is_started(self):
        return self.WatchDogCounterStarted

    def step_watch_dog_counter(self):
        self.WatchDogCounter -= 1
        if self.WatchDogCounter <= 0:
            self.WatchDogCounterFinished = True
        return self.WatchDogCounter

    def watch_dog_counter_is_finished(self):
    ...
    def get_current_time_in_msec(self):
    ...
    def get_elapsed_time_in_msec(self):
    ...
    def get_execution_time_in_msec(self):
    ...
    def get_bar_date_time(self, BarIndex):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return V[i].Date

    def get_bar_date(self, BarIndex):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return V[i].Date

    def get_bar_time(self, BarIndex):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return V[i].Date

    def is_bar(self, BarIndex, BarZamani):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return V[i].Date.strftime("%H:%M") == BarZamani

    def is_poz_kapat_valid(self, BarIndex, PozKapatZamani):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return V[i].Date.strftime("%H:%M") == PozKapatZamani

    def check_bar_time_with(self, BarIndex, BarZamani):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return self.compare_times(self.get_bar_date_time(i), self.get_time(BarZamani))

    def check_bar_date_with(self, BarIndex, BarZamani):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return self.compare_dates(self.get_bar_date_time(i), self.get_date(BarZamani))

    def check_bar_date_time_with(self, BarIndex, BarZamani):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return self.compare_dates(self.get_bar_date_time(i), self.get_date_time(BarZamani))

    def is_gun_sonu_poz_kapat_sinyali_geldi_sadece_analiz(self, BarIndex):
        i = BarIndex
        V = Sistem.GrafikVerileri
        BarCount = len(V)
        return (i < BarCount - 1 and V[i].Date.day != V[i + 1].Date.day)

    def is_ay_sonu_poz_kapat_sinyali_geldi_sadece_analiz(self, BarIndex):
        i = BarIndex
        V = Sistem.GrafikVerileri
        BarCount = len(V)
    ...
    def get_week_number_from_bar_index(self, BarIndex):
        i = BarIndex
        V = Sistem.GrafikVerileri
        return self.get_week_number(V[i].Date)
