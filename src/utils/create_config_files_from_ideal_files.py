...
class ConfigFileConverter:
    """
    iDeal Config dosyalarını parse edip CSV/JSON formatına dönüştüren sınıf.
    """
    ...
    def __init__(self,
        ...
        self.symbols_dir_txt_files = symbols_dir_txt_files
        self.symbols_dir_csv_files = symbols_dir_csv_files

    def parse_teminatlar_txt(self):
        """Teminatlar.txt dosyasını parse eder ve CSV formatında kaydeder"""
        input_file = os.path.join(self.source_dir, "Teminatlar.txt")
        output_file = os.path.join(self.target_dir, "teminatlar.csv")
    ...
    def parse_imkb_endeks_senetler_txt(self):
        """ImkbEndeksSenetler.txt dosyasını parse eder ve CSV formatında kaydeder"""
        input_file = os.path.join(self.source_dir, "ImkbEndeksSenetler.txt")
        output_file = os.path.join(self.target_dir, "bistEndeksBilesenleri.csv")
    ...
    def parse_imkb_endeksler_txt(self):
        """ImkbEndeksler.txt dosyasını parse eder ve CSV formatında kaydeder"""
        input_file = os.path.join(self.source_dir, "ImkbEndeksler.txt")
        output_file = os.path.join(self.target_dir, "bistEndeksleri.csv")
    ...
    def parse_semboller_txt(self):
        """_Semboller.txt dosyasını parse eder ve CSV formatında kaydeder"""
        input_file = os.path.join(self.source_dir, "_Semboller.txt")
        output_file = os.path.join(self.target_dir, "semboller.csv")
    ...
    def build_sembol_istatiskleri_from_txt_files(self):
        csv_output_file = os.path.join(self.target_dir, "sembolIstatistikleri.csv")
        json_output_file = os.path.join(self.target_dir, "sembolIstatistikleri.json")

        # Hedef dizini oluştur
        os.makedirs(self.target_dir, exist_ok=True)

        print(f"Scanning chart data files in {self.symbols_dir_txt_files}...")

        # Sembol bilgilerini tutacak map
        symbol_data_map = {}

        # ChartData/_Exports dizinindeki tüm .txt dosyalarını tara
        if os.path.exists(self.symbols_dir_txt_files):
            txt_files = glob.glob(os.path.join(self.symbols_dir_txt_files, "*.txt"))
    ...
    def build_sembol_istatiskleri_from_csv_files(self):
        csv_output_file = os.path.join(self.target_dir, "sembolIstatistikleri.csv")
        json_output_file = os.path.join(self.target_dir, "sembolIstatistikleri.json")
        ...
        print(f"Scanning CSV files in {self.symbols_dir_csv_files}...")

        # Sembol bilgilerini tutacak map
        symbol_data_map = {}

        # csvFiles dizinindeki tüm alt klasörleri ve CSV dosyalarını tara
        if os.path.exists(self.symbols_dir_csv_files):
            all_csv_files = []

            # Tüm CSV dosyalarını bul
            for root, dirs, files in os.walk(self.symbols_dir_csv_files):
                for file in files:
            ...
            for i, file_path in enumerate(all_csv_files, 1):
                ...
                relative_path = os.path.relpath(file_path, self.symbols_dir_csv_files)
    ...
    def convert_files(self):
        ...
        self.parse_semboller_txt()

        # ImkbEndeksler.txt'yi işle
        self.parse_imkb_endeksler_txt()

        # ImkbEndeksSenetler.txt'yi işle
        self.parse_imkb_endeks_senetler_txt()

        # Teminatlar.txt'yi işle
        self.parse_teminatlar_txt()
...
if __name__ == "__main__":
    ...
    converter = ConfigFileConverter(
        ...
        symbols_dir_txt_files=symbols_dir_txt_files,
        symbols_dir_csv_files=symbols_dir_csv_files
    )
